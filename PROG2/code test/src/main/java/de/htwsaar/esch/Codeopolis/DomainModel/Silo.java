 ...
 * For (Übung 4)
 * we had to modify Silo as well so that the values are stored in Linked-list in case we want to
 * modify the size of the list dynamically
 * see below we added (  private LinkedList<Harvest> stock;) and in most of the functions so that we make sure that the linked list is used
 *
 */
public class Silo implements Serializable, Comparable<Silo> {
    ...
    public Silo(Silo other) {
        ...
        for (int i = 0; i <= other.stockIndex; i++) {
            this.stock.set(i, other.stock.get(i).copy());
        }
    }
    ...
    public Harvest store(Harvest harvest) {
        ...
        if (fillLevel < capacity) {
            ...
            if (harvest.getAmount() <= remainingCapacity) {
                this.stockIndex++;
                this.stock.set(this.stockIndex, harvest);
                ...
                this.stockIndex++;
                this.stock.set(this.stockIndex, remainingHarvest); // Store the remaining harvest in the current depot
